...
@section('content')
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <div class="d-flex align-items-center">
                        <div class="ml-auto d-flex gap-2" style="gap: 0.8rem;">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">@lang('Dashboard')</a></li>
                                <li class="breadcrumb-item active">@lang('Expenses Report')</li>
                            </ol>
    ...
    <!-- Desktop Content Header (like income-report) -->
    ...
    <!-- Expense Report Card Section -->
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-info">
                    <h3 class="card-title">@lang('Expense Report')</h3>
                </div>
                <div class="card-body">
                    <!-- Filter Form Section -->
                    <form id="filter"  class="collapse" action="{{ route('expense-report.index') }}" method="get">
                        <div class="mobile-form-container">
                            ...
                            <div class="mobile-form-row">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="amount" id="amount" placeholder="@lang('Amount')" class="form-control" value="{{ request('amount') }}">
                                </div>
                                <div class="form-group">
                                    <label for="description">@lang('Description')</label>
                                    <input type="text" name="description" id="description" placeholder="@lang('Description')" class="form-control" value="{{ old('description', request()->description) }}">
                                </div>
                            </div>
                            ...
                            <div class="submit-container">
                                ...
                                @if(request()->hasAny(['date_from', 'date_to', 'amount', 'account_id', 'expense_category_id', 'description']))
                                    <a href="{{ route('expense-report.index') }}" class="btn btn-secondary">@lang('Clear')</a>
                                @endif
                            </div>
                        </div>
                    </form>

                    <!-- Expense Items Table Section -->
                    <div class="card">
                        <div class="card-header bg-info">
                            <h3 class="card-title">@lang('Expense Items')</h3>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered custom-table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="background: linear-gradient(to right, rgba(255,255,255,0) 0%, {{ $categoryColors[$item->expense_category_id] ?? '#FFFFFF' }} 50%, rgba(255,255,255,0) 100%);">
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $item->account->account_title ?? '-' }}</td>
                                            <td>{{ $item->expenseCategory->title ?? '-' }}</td>
