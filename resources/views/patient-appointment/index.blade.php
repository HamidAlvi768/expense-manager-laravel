...
@section('content')
    ...
    @php
        ...
        $appointmentDate = \Carbon\Carbon::parse($patientAppointment->appointment_date); 
    @endphp

    @if ($currentDate->lessThanOrEqualTo($appointmentDate))  <!-- Only show if appointment date is in the future --> 
        @if (!$patientAppointment->email_sent)

       <?php if(strpos($patientAppointment->user->email, 'noemail') === false) { ?>
        
        
           
            <button class="btn btn-primary btn-send-reminder" data-id="{{ $patientAppointment->id }}" id="reminderBtn-{{ $patientAppointment->id }}">
                Send Reminder {{strpos($patientAppointment->user->email, 'noemail')}}
            </button>
        <?php } ?>   
        @else
        ...
        @endif
    @else
        <!-- Optionally display a message like 'Appointment Passed' -->
...
<script>
   $(document).on('click', '.btn-send-reminder', function () {
    ...
    $.ajax({
        url: '/send-reminder',  // Adjust the route as per your setup
        method: 'POST',
        data: {
            id: appointmentId,
            _token: '{{ csrf_token() }}'
