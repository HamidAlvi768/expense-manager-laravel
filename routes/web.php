...
Route::group(['middleware' => ['auth']], function() {
    ...
    Route::resources([
        ...
        'dd-income-category' => App\Http\Controllers\DdIncomeCategoryController::class,
        'accounts' => App\Http\Controllers\AccountController::class,
        'dd-account-type' => App\Http\Controllers\DdAccountTypeController::class,
        'expenses' => App\Http\Controllers\ExpenseController::class,
        'dd-expense-category' => App\Http\Controllers\DdExpenseCategoryController::class,
        'transfers' => App\Http\Controllers\TransferController::class,
        'bank-statements' => App\Http\Controllers\BankStatementController::class,
        'keywords' => App\Http\Controllers\KeywordController::class,
        'budgets' => App\Http\Controllers\BudgetController::class,

    ]);

    Route::get('transfer-report', [App\Http\Controllers\ReportController::class, 'transferReportIndex'])->name('transfer-report.index');
    Route::get('income-report', [App\Http\Controllers\ReportController::class, 'incomeReportIndex'])->name('income-report.index');
    Route::get('expense-report', [App\Http\Controllers\ReportController::class, 'expenseReportIndex'])->name('expense-report.index');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('/dashboard/get-expense-donut-data', [App\Http\Controllers\DashboardController::class, 'getExpenseDonutData']);
    Route::get('/dashboard/income-expense', [App\Http\Controllers\DashboardController::class, 'incomeExpenseData'])->name('report.incomeExpenseData');
    Route::get('/dashboard/update-counts', [App\Http\Controllers\DashboardController::class, 'updateDashboardCounts'])->name('dashboard.updateCounts');
    Route::get('fetching-categories', [App\Http\Controllers\KeywordController::class, 'getCategories'])->name('keywords.getCategories');
    Route::get('/dashboard/get-budget-vs-expenses', [App\Http\Controllers\DashboardController::class, 'getBudgetVsExpenses']);
    Route::get('/expenses/{id}/mark-important', [App\Http\Controllers\ExpenseController::class, 'markImportant'])->name('expenses.markImportant');
...
Route::get('/events/details', [App\Http\Controllers\EventController::class, 'fetchEventDetails'])->name('events.details');
Route::get('/events/fetch', [App\Http\Controllers\EventController::class, 'fetchEvents'])->name('events.fetch');
...
// Added specifically for email testing on exceptions.
Route::get('/test-exception', function () {
    throw new \Illuminate\Database\QueryException(
        "SQLSTATE[HY000]: General error: 1364 Field 'updated_by' doesn't have a default value",
        [],
        new Exception()
    );
});
...
Route::get('/run-clear-cache', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'clearAndCache']);
...
Route::post('/forgot-password', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'sendResetEmail'])
    ->middleware('throttle:5,1') // 5 requests per minute per user
    ->name('forgot-password.email');
Route::get('/reset-password/{token}', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'showResetPasswordForm'])->name('reset-password.form');
Route::post('/reset-password', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'updatePassword'])->name('reset-password.update');
